...
class Econ():
	...
	def CalcGridDemand(self, gridDemand, mode, gridDemandEC=[]):
		...
		if mode == "EC":
			...
			return (sum(gridDemandEC) * self.priceDemand) + difference * ((self.priceDemand + self.priceFeedIn)/2)
	...
	def CalcGridFeedIn(self, gridFeedIn, mode, gridFeedInEC=[]):
		...
		if mode == "EC":
			...
			return (sumgridFeedInEC * self.priceFeedIn) + difference * ((self.priceDemand + self.priceFeedIn)/2)
	...
	def CalcNetMetering(self, gridDemand, gridFeedIn):
		...
		return energycosts   
	def CalcNetMetering2(self, gridDemand, gridFeedIn):
